...
def get_cfa_image_desires_with_greater_price(db: Session, cfa_image_id: int, price: float, user_id: int) -> List[models.Desire]:
    desires = db.query(models.Desire).filter(and_(
        models.Desire.count > 0,
        models.Desire.buyer_id != user_id,
        models.Desire.cfa_image_id == cfa_image_id,
        models.Desire.price >= price
    )).order_by(models.Desire.price.desc()).all()

...
def get_cfa_image_offers_with_less_price(db: Session, cfa_image_id: int, price: float, user_id: int) -> List[models.Offer]:
    offers = db.query(models.Offer).filter(and_(
        models.Offer.count > 0,
        models.Offer.seller_id != user_id,
        models.Offer.cfa_image_id == cfa_image_id,
        models.Offer.price <= price
    )).order_by(models.Offer.price.asc()).all()
